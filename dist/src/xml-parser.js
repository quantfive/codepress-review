 ...
 */
function parseAgentResponse(xmlText) {
    const findings = [];
    ...
    const resolvedSection = xmlText.match(/<resolvedComments>([\s\S]*?)<\/resolvedComments>/s);
    if (resolvedSection) {
        ...
        while ((match = resolvedRegex.exec(resolvedSection[1])) !== null) {
            ...
            const reasonMatch = resolvedContent.match(/<reason>([\s\S]*?)<\/reason>/s);
            const commentIdMatch = resolvedContent.match(/<commentId>(.*?)<\/commentId>/s);
