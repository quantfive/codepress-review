...
async function callWithRetry(fn, hunkIdx) {
    ...
    while (attempt < MAX_RETRIES) {
        try {
            ...
            (0, debug_1.debugWarn)(`[Hunk ${hunkIdx}] Attempt ${attempt} failed: ${error}. Retrying in ${wait}ms...`);
 ...
 */
async function summarizeDiff(chunks, modelConfig, existingReviews = [], existingComments = [], blockingOnly = false) {
    ...
    if (existingReviews.length > 0) {
        const botReviews = existingReviews.filter((review) => (0, constants_1.isCodePressReviewObject)(review) && review.body);
    ...
    if (existingComments.length > 0) {
        const botComments = existingComments.filter((comment) => (0, constants_1.isCodePressCommentObject)(comment));
    ...
    const systemPrompt = (0, summary_agent_system_prompt_1.getSummarySystemPrompt)(blockingOnly);
    const userContent = `
<summaryRequest>
...
${diffOverview}
    ...
    (0, debug_1.debugLog)("🔍 User content:", userContent);
    (0, debug_1.debugLog)("🔍 System prompt:", systemPrompt);
    let text;
    try {
        ...
        text = result.text;
        (0, debug_1.debugLog)("✅ AI call succeeded for diff summary", result);
    }
    catch (error) {
        (0, debug_1.debugLog)("❌ AI call failed for diff summary:", error);
        throw error;
    }
    (0, debug_1.debugLog)("Diff Summary Raw Response:", text);
...
function parseSummaryResponse(text) {
    try {
        ...
        if (decisionMatch) {
            const recommendationMatch = decisionMatch[1].match(/<recommendation>(.*?)<\/recommendation>/s);
            const reasoningMatch = decisionMatch[1].match(/<reasoning>(.*?)<\/reasoning>/s);
        ...
        const prDescriptionMatch = globalContent.match(/<prDescription>(.*?)<\/prDescription>/s);
        const prDescription = prDescriptionMatch
            ? normalizeIndentation(prDescriptionMatch[1])
            : undefined;
        (0, debug_1.debugLog)(`🔍 PR Description parsing:`, {
        ...
        if (!prDescriptionMatch) {
            (0, debug_1.debugLog)("🔍 Full AI Response (no prDescription found):", text);
        }
        // Extract hunks
        ...
        const plan = (() => {
            ...
            // globalBudget
            const gb = {};
            const gbMatch = planContent.match(/<globalBudget>([\s\S]*?)<\/globalBudget>/s);
            if (gbMatch) {
                const gbBody = gbMatch[1];
                const num = (s) => s && /\d+/.test(s.trim()) ? parseInt(s.trim(), 10) : undefined;
                ...
                const defaultMaxTurns = gbBody.match(/<defaultMaxTurns>([\s\S]*?)<\/defaultMaxTurns>/s);
                const sequentialTop = gbBody.match(/<sequentialTop>([\s\S]*?)<\/sequentialTop>/s);
                const maxConcurrentHunks = gbBody.match(/<maxConcurrentHunks>([\s\S]*?)<\/maxConcurrentHunks>/s);
            ...
            function parseParams(paramsBlock) {
                const out = {};
                const regex = /<([a-zA-Z0-9_]+)>([\s\S]*?)<\/\1>/g;
                let m = regex.exec(paramsBlock);
                while (m !== null) {
                    const key = m[1];
                    const raw = m[2].trim();
            ...
            if (planHunksMatch) {
                const hunkBlocks = planHunksMatch[1].match(/<hunk[^>]*>[\s\S]*?<\/hunk>/g) || [];
                for (const hb of hunkBlocks) {
                    ...
                    if (actionsBlock) {
                        const actionBlocks = actionsBlock.match(/<action[^>]*>[\s\S]*?<\/action>/g) || [];
                        for (const ab of actionBlocks) {
                            const toolMatch = ab.match(/<action[^>]*tool="([^"]+)"/);
                            const goalMatch = ab.match(/<goal>([\s\S]*?)<\/goal>/s);
                            const paramsBlock = ab.match(/<params>([\s\S]*?)<\/params>/s)?.[1];
                            const action = {
                                tool: toolMatch?.[1]?.trim(),
                                goal: goalMatch?.[1]?.trim() || "",
                            };
                            if (paramsBlock)
                                action.params = parseParams(paramsBlock);
                            actions.push(action);
                        }
                    }
                    const hp = {
                        index,
                        riskLevel: riskLevel,
                        priority: priority && /^-?\d+$/.test(priority)
                            ? parseInt(priority, 10)
                            : undefined,
                        maxTurns: maxTurns && /^-?\d+$/.test(maxTurns)
                            ? parseInt(maxTurns, 10)
                            : undefined,
                        toolBudget: toolBudget && /^-?\d+$/.test(toolBudget)
 ...
 */
async function summarizeFindings(required, optional, nit, fyi, praise, modelConfig) {
    const model = await (0, model_factory_1.createModel)(modelConfig);
...
${others.length > 0 ? "<othersSummary>Write a paragraph summarizing the optional, nit, and fyi findings</othersSummary>" : ""}
    ...
    try {
        ...
        text = result.text;
        (0, debug_1.debugLog)("✅ AI call succeeded for findings summary");
    }
    catch (error) {
        (0, debug_1.debugLog)("❌ AI call failed for findings summary:", error);
        throw error;
    }
    (0, debug_1.debugLog)("Findings Summary Raw Response:", text);
    // Parse the response
    const result = {};
    try {
        const praiseSummaryMatch = text.match(/<praiseSummary>(.*?)<\/praiseSummary>/s);
        if (praiseSummaryMatch) {
            result.praiseSummary = praiseSummaryMatch[1].trim();
        }
        const requiredSummaryMatch = text.match(/<requiredSummary>(.*?)<\/requiredSummary>/s);
        if (requiredSummaryMatch) {
            result.requiredSummary = requiredSummaryMatch[1].trim();
        }
        const othersSummaryMatch = text.match(/<othersSummary>(.*?)<\/othersSummary>/s);
