...
const pr_files_1 = require("./pr-files");
function getTriggerEvent(context, config) {
...
function shouldRunAction(context, config) {
    const eventName = context.eventName;
    const action = context.payload.action;
    // PR opened
    if (eventName === "pull_request" && action === "opened") {
...
async function run() {
    try {
        ...
        const runOnReviewRequested = core.getBooleanInput("run_on_review_requested");
        ...
        try {
            ...
            const existingCommentsFile = (0, node_path_1.resolve)("pr-comments.json");
            const botCommentsFile = (0, node_path_1.resolve)("bot-comments.json");
            try {
                ...
                const isBotComment = (comment) => comment.user &&
                ...
                const botComments = reviewComments
                ...
                (0, node_fs_1.writeFileSync)(existingCommentsFile, JSON.stringify(existingComments, null, 2));
                (0, node_fs_1.writeFileSync)(botCommentsFile, JSON.stringify(botComments, null, 2));
                core.info(`Found ${existingComments.length} review comments from other reviewers`);
                if (botComments.length > 0) {
                    core.info(`Found ${botComments.length} of your own previous comments for deduplication`);
                }
            }
            catch (commentError) {
                core.warning(`Failed to fetch existing comments (continuing without them): ${commentError}`);
                (0, node_fs_1.writeFileSync)(existingCommentsFile, "[]");
                (0, node_fs_1.writeFileSync)(botCommentsFile, "[]");
            }
            // Fetch PR files and filter out ignored patterns (lock files, build outputs, etc.)
            const prFilesFile = (0, node_path_1.resolve)("pr-files.json");
            try {
                ...
                const allFiles = prFilesData.map((file) => ({
                ...
                const filteredFiles = (0, pr_files_1.filterPRFiles)(allFiles);
                const filterStats = (0, pr_files_1.getFilterStats)(allFiles.length, filteredFiles);
                const includePatches = (0, pr_files_1.shouldIncludePatches)(filteredFiles);
                const formattedFiles = (0, pr_files_1.formatPRFilesForPrompt)(filteredFiles, includePatches);
                // Save both raw filtered files and formatted prompt section
                (0, node_fs_1.writeFileSync)(prFilesFile, JSON.stringify({
                    files: filteredFiles,
                ...
                (0, node_fs_1.writeFileSync)(prFilesFile, JSON.stringify({ files: [], formatted: "", originalCount: 0, filteredCount: 0 }, null, 2));
            }
            ...
            let previousReviewCommitSha = null;
            try {
                ...
                const botReviews = reviews
                    ...
                    .sort((a, b) => new Date(b.submitted_at || 0).getTime() -
                    new Date(a.submitted_at || 0).getTime());
                if (botReviews.length > 0) {
                    const lastBotReview = botReviews[0];
                    previousReviewState = lastBotReview.state;
                    previousReviewCommitSha = lastBotReview.commit_id || null;
                    core.info(`Found previous bot review: ${previousReviewState} at commit ${previousReviewCommitSha?.substring(0, 7)}`);
                }
            }
            catch (reviewError) {
                core.warning(`Failed to fetch previous reviews (continuing without them): ${reviewError}`);
            }
            ...
            if (isReReview) {
                ...
                if (previousReviewCommitSha && previousReviewCommitSha !== commitSha) {
                    core.info(`Commits changed: ${previousReviewCommitSha.substring(0, 7)} → ${commitSha.substring(0, 7)}`);
                }
            }
        }
        catch (error) {
        ...
        if (relatedReposInput.trim()) {
            try {
                ...
                if (relatedRepos.length > 0) {
                    const tempDir = (0, node_fs_2.mkdtempSync)((0, node_path_2.join)((0, node_os_1.tmpdir)(), "codepress-related-"));
                    ...
                    for (const relatedRepo of relatedRepos) {
                        ...
                        const localPath = (0, node_path_2.join)(tempDir, repoName);
                        const ref = relatedRepo.ref || "main";
                        try {
                            const cloneUrl = `https://x-access-token:${relatedReposToken}@github.com/${relatedRepo.repo}.git`;
                            (0, node_child_process_1.execSync)(`git clone --depth 1 --branch ${ref} "${cloneUrl}" "${localPath}"`, { stdio: "pipe" });
