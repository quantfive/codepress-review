...
async function reviewChunkWithAgent(diffChunk, modelConfig, diffSummary, chunkIndex, repoFilePaths, existingComments = [], maxTurns = 20, blockingOnly = false) {
    ...
    if (diffSummary) {
        ...
        if (hunkSummary) {
            ...
            if (diffSummary.plan) {
                const planForHunk = diffSummary.plan.hunkPlans.find((hp) => hp.index === chunkIndex);
                if (planForHunk) {
                    contextLines.push("    <plan>");
                    if (planForHunk.riskLevel) {
                        contextLines.push(`      <riskLevel>${planForHunk.riskLevel}</riskLevel>`);
                    }
                    if (typeof planForHunk.priority === "number") {
                        contextLines.push(`      <priority>${String(planForHunk.priority)}</priority>`);
                    }
                    if (typeof planForHunk.maxTurns === "number") {
                        contextLines.push(`      <maxTurns>${String(planForHunk.maxTurns)}</maxTurns>`);
                    }
                    if (typeof planForHunk.toolBudget === "number") {
                        contextLines.push(`      <toolBudget>${String(planForHunk.toolBudget)}</toolBudget>`);
                    }
                    if (typeof planForHunk.skip === "boolean") {
                        contextLines.push(`      <skip>${planForHunk.skip ? "true" : "false"}</skip>`);
                    }
                    if (Array.isArray(planForHunk.focus) &&
                    ...
                    if (typeof planForHunk.evidenceRequired === "boolean") {
                        contextLines.push(`      <evidenceRequired>${planForHunk.evidenceRequired ? "true" : "false"}</evidenceRequired>`);
                    }
                    if (Array.isArray(planForHunk.actions) &&
