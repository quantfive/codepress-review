...
**Remember: You MUST submit a formal review at the end using \`gh pr review\`.`}
    ...
    runner.on("agent_tool_end", (_context, _agent, tool, result, details) => {
        ...
        if (tool.name === "complete_review" && result) {
            ...
            const typedResult = resultObj;
            if (typedResult.completed) {
                reviewCompletion = {
                    completed: true,
                    verdict: typedResult.verdict || "NONE",
                    summary: typedResult.summary || "",
                    commentsPosted: reviewState.commentsPostedThisRun.length,
                };
                (0, debug_1.debugLog)("[Review] complete_review tool called - review will terminate");
            }
        }
    });
    try {
        ...
        if (completion) {
            (0, debug_1.debugLog)("Agent completed review via complete_review tool.");
            (0, debug_1.debugLog)(`  Verdict: ${completion.verdict}`);
            (0, debug_1.debugLog)(`  Summary: ${completion.summary}`);
            (0, debug_1.debugLog)(`  Comments posted: ${completion.commentsPosted}`);
        }
        else {
            (0, debug_1.debugLog)("Agent completed without calling complete_review tool.");
