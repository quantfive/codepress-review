 ...
 */
const ReviewCompletionSchema = zod_1.z.object({
    /** Must be true to signal completion - z.literal(true) prevents premature termination */
    completed: zod_1.z.literal(true).describe("Must be true to signal the review is complete"),
    /** Brief summary of the review outcome */
    summary: zod_1.z.string().describe("A brief summary of the review: what was found, comments posted, and final verdict"),
    /** Number of comments posted during this review */
    commentsPosted: zod_1.z.number().describe("Number of inline comments posted during this review"),
    /** The review verdict submitted */
    verdict: zod_1.z.enum(["APPROVE", "REQUEST_CHANGES", "COMMENT", "NONE"]).describe("The verdict submitted with gh pr review, or NONE if no review was submitted"),
});
/**
 ...
 */
function formatBotPreviousComments(comments) {
    if (comments.length === 0) {
        return "";
    }
 ...
 */
function formatRelatedRepos(repos) {
    if (repos.length === 0) {
        return "";
    }
 ...
 */
function createInterventionFilter(state) {
    return (args) => {
        // Advance turn counter
        (0, review_state_1.advanceTurn)(state);
        (0, debug_1.debugLog)(`Turn ${state.currentTurn}${state.maxTurns ? `/${state.maxTurns}` : ""}`);
        // Check if we should force text-only response (at max turns)
        if ((0, interventions_1.shouldForceTextOnly)(state)) {
            (0, debug_1.debugLog)("At max turns - forcing text-only response");
...
${interventions_1.MAX_STEPS_PROMPT}
...
${args.modelData.instructions || ""}`;
        ...
        const interventionBlock = (0, interventions_1.generateInterventionBlock)(state);
        if (interventionBlock) {
            (0, debug_1.debugLog)("Injecting intervention reminder");
            // Find the last user message and prepend the intervention
            const lastUserMsgIndex = args.modelData.input
                .map((item, i) => ({ item, i }))
                .filter((x) => x.item.role === "user")
                .pop()?.i;
            if (lastUserMsgIndex !== undefined) {
                const lastUserMsg = args.modelData.input[lastUserMsgIndex];
 ...
 */
async function reviewFullDiff(modelConfig, repoFilePaths, prContext, maxTurns = null, // null = unlimited (default)
blockingOnly = false, existingComments = [], botPreviousComments = [], relatedRepos = [], prFilesFormatted = "") {
    // Reset todo list for fresh review
    (0, tools_1.resetTodoList)();
    // Initialize review state for intervention tracking
    const reviewState = (0, review_state_1.createReviewState)({
        maxTurns,
        botPreviousComments,
    });
    ...
    if (existingComments.length > 0) {
        (0, debug_1.debugLog)(`📝 Including ${existingComments.length} existing review comments from other reviewers`);
    }
    ...
    if (botPreviousComments.length > 0) {
        (0, debug_1.debugLog)(`🔄 Including ${botPreviousComments.length} of your own previous comments for deduplication`);
    }
    // Format related repos context
    const relatedReposSection = formatRelatedRepos(relatedRepos);
    if (relatedRepos.length > 0) {
        (0, debug_1.debugLog)(`🔗 Including ${relatedRepos.length} related repo(s) for cross-repo context`);
    }
...
**Remember: You MUST submit a formal review at the end using \`gh pr review\`.`}
    ...
    runner.on("agent_tool_start", (_context, _agent, tool, _details) => {
        (0, review_state_1.recordToolCall)(reviewState, tool.name);
        (0, debug_1.debugLog)(`Tool start: ${tool.name}`);
    });
    runner.on("agent_tool_end", (_context, _agent, tool, result, details) => {
        ...
        (0, interventions_1.analyzeToolOutput)(reviewState, tool.name, details.toolCall, result);
    });
    try {
        (0, debug_1.debugLog)(`Starting agentic PR review`);
        (0, debug_1.debugLog)(`Max turns: ${maxTurns === null ? "unlimited" : maxTurns}`);
        (0, debug_1.debugLog)(`PR: ${prContext.repo}#${prContext.prNumber}`);
        (0, debug_1.debugLog)(`Repository files available: ${repoFilePaths.length}`);
        if (botPreviousComments.length > 0) {
            (0, debug_1.debugLog)(`Bot previous comments for deduplication: ${botPreviousComments.length}`);
        }
        // Build run options
        ...
        if (result.finalOutput) {
            (0, debug_1.debugLog)("Agent completed review with structured output.");
            (0, debug_1.debugLog)(`  Verdict: ${result.finalOutput.verdict}`);
            (0, debug_1.debugLog)(`  Summary: ${result.finalOutput.summary}`);
            (0, debug_1.debugLog)(`  Comments posted (self-reported): ${result.finalOutput.commentsPosted}`);
            (0, debug_1.debugLog)(`  Comments posted (tracked): ${reviewState.commentsPostedThisRun.length}`);
        }
        else {
            (0, debug_1.debugLog)("Agent completed without structured output (unexpected).");
            (0, debug_1.debugLog)(`  Comments posted this run: ${reviewState.commentsPostedThisRun.length}`);
            (0, debug_1.debugLog)(`  Review submitted: ${reviewState.hasSubmittedReview}`);
        }
    }
    catch (error) {
