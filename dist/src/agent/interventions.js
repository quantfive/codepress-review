...
exports.MAX_STEPS_PROMPT = void 0;
exports.generateInterventions = generateInterventions;
exports.wrapWithSystemReminder = wrapWithSystemReminder;
exports.generateInterventionBlock = generateInterventionBlock;
exports.shouldForceTextOnly = shouldForceTextOnly;
exports.analyzeToolOutput = analyzeToolOutput;
 ...
 */
function generateInterventions(state) {
    const interventions = [];
    const progress = (0, review_state_1.calculateProgress)(state);
    ...
    if (state.maxTurns !== null) {
        ...
        if (turnsRemaining <= 3 && turnsRemaining > 0) {
            interventions.push(`⚠️ **TURN BUDGET WARNING**: You have only ${turnsRemaining} turn${turnsRemaining === 1 ? "" : "s"} remaining. ` +
        ...
        if (turnsRemaining === 0) {
            interventions.push(`🛑 **FINAL TURN**: This is your last turn. You MUST:\n` +
    ...
    if (state.currentTurn >= 3 &&
        ...
        interventions.push(`📝 **CRITICAL: Check your previous comments**. You have ${state.botPreviousComments.length} previous comment(s) ` +
 ...
 */
function wrapWithSystemReminder(content) {
    if (!content.trim())
        return "";
 ...
 */
function generateInterventionBlock(state) {
 ...
 */
exports.MAX_STEPS_PROMPT = `I need to complete this review now. I will respond with my final assessment without making any more tool calls.`;
/**
 * Checks if we should inject the max steps prompt (force text-only response).
 * This is only applicable when maxTurns is set.
 */
function shouldForceTextOnly(state) {
    if (state.maxTurns === null)
        return false;
 ...
 */
function analyzeToolOutput(state, toolName, toolInput, _toolOutput) {
