...
exports.resolveModelAliasDynamic = resolveModelAliasDynamic;
 ...
 */
async function resolveModelAliasDynamic(provider, modelName, apiKey) {
 ...
 */
function resolveModelAlias(provider, modelName) {
    const normalizedProvider = provider.toLowerCase();
    const normalizedModel = modelName.toLowerCase().replace(/\s+/g, "-").trim();
    const fallback = FALLBACK_MODEL_ALIASES[normalizedProvider]?.[normalizedModel];
    if (fallback) {
