...
class ReviewService {
    ...
    async processChunk(chunk, chunkIndex, existingComments) {
        ...
        try {
            ...
            findings = await (0, ai_client_1.callWithRetry)(() => (0, agent_1.reviewChunkWithAgent)(chunk.content, modelConfig, summaryContext, this.repoFilePaths, this.config.maxTurns), chunkIndex + 1);
        }
        catch (error) {
    ...
    async execute() {
        ...
        if (filteredChunks.length > 0) {
            ...
            try {
                const modelConfig = (0, config_1.getModelConfig)();
                this.diffSummary = await (0, ai_client_1.callWithRetry)(() => (0, ai_client_1.summarizeDiff)(filteredChunks.map(({ chunk }) => chunk), modelConfig), 0);
