...
class ReviewService {
    ...
    async processChunk(chunk, chunkIndex, existingComments, existingCommentsData) {
        ...
        if (relevantComments.length > 0) {
            console.log(`[Hunk ${chunkIndex + 1}] Found ${relevantComments.length} existing comments for this chunk - passing to agent for context`);
        }
        let findings = [];
        try {
            const modelConfig = (0, config_1.getModelConfig)();
            const agentResponse = await (0, ai_client_1.callWithRetry)(() => (0, agent_1.reviewChunkWithAgent)(chunk.content, modelConfig, this.diffSummary, chunkIndex, this.repoFilePaths, relevantComments, this.config.maxTurns), chunkIndex + 1);
            findings = agentResponse.findings;
            // Add detailed logging for raw findings from the agent
            console.log(`[Hunk ${chunkIndex + 1}] Raw agentResponse:`, JSON.stringify(agentResponse, null, 2));
            // Handle resolved comments from agentResponse.resolvedComments
            if (agentResponse.resolvedComments.length > 0) {
                console.log(`[Hunk ${chunkIndex + 1}] Found ${agentResponse.resolvedComments.length} comments to resolve:`, agentResponse.resolvedComments.map((rc) => `${rc.path}:${rc.line} - ${rc.reason}`));
        ...
        const uniqueFindings = findings.filter((finding) => {
            ...
            if (existingComments.get(finding.path)?.has(finding.line)) {
                console.log(`[Hunk ${chunkIndex + 1}] Skipping finding on ${finding.path}:${finding.line} as a comment already exists on this line.`);
                return false;
            }
    ...
    async execute() {
        ...
        if (filteredChunks.length > 0) {
            console.log("Performing initial diff summarization with existing context...");
            try {
                const modelConfig = (0, config_1.getModelConfig)();
                this.diffSummary = await (0, ai_client_1.callWithRetry)(() => (0, ai_client_1.summarizeDiff)(filteredChunks.map(({ chunk }) => chunk), modelConfig, existingReviews, existingCommentsData), 0);
                ...
                if (this.config.updatePrDescription &&
                    ...
                    await this.githubClient.updatePRDescription(this.config.pr, this.diffSummary.prDescription.trim());
                }
            }
            catch (error) {
        ...
        if (shouldCreateReview) {
            ...
            console.log("🎉 No issues found during review and no decision available!");
        }
    }
}
