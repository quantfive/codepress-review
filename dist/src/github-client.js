    ...
    async updatePRDescription(prNumber, description) {
        try {
            ...
            if (!currentDescription || currentDescription.trim() === "") {
                ...
                console.log(`⏭️  PR #${prNumber} already has a description, skipping update`);
                return false;
            }
        }
        catch (error) {
    ...
    async createReview(prNumber, commitId, findings, diffSummary) {
        ...
        if (praise.length > 0 || required.length > 0 || others.length > 0) {
            try {
                ...
                console.warn("Failed to generate finding summaries, falling back to detailed list:", error);
            }
        }
        ...
        try {
            ...
            const eventText = reviewEvent === "APPROVE"
                ...
                : reviewEvent === "REQUEST_CHANGES"
                    ? "requested changes"
                    : "commented on";
            console.log(`✅ ${eventText.charAt(0).toUpperCase() + eventText.slice(1)} PR with ${findings.length} comments`);
        }
        catch (error) {
            await this.rateLimitHandler.handleRateLimit(error, makeRequest);
            const eventText = reviewEvent === "APPROVE"
                ...
                : reviewEvent === "REQUEST_CHANGES"
                    ? "requested changes"
                    : "commented on";
            console.log(`✅ ${eventText.charAt(0).toUpperCase() + eventText.slice(1)} PR with ${findings.length} comments (after retry)`);
        }
    }
