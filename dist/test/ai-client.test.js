  ...
  <hunk index="1">
    ...
    <tests>
      <item>Verify documentation accuracy</item>
            ...
            const result = await (0, ai_client_1.summarizeDiff)(mockChunks, mockModelConfig);
            ...
            expect(generateText).toHaveBeenCalledWith({
                ...
                prompt: expect.stringContaining('<chunk index="0" file="src/example.ts">'),
            });
        });
        it("should handle file-based custom prompt in summarizeDiff", async () => {
            ...
            await (0, ai_client_1.summarizeDiff)(mockChunks, mockModelConfig);
            expect(require("../src/summary-agent-system-prompt").getSummarySystemPrompt).toHaveBeenCalledWith(false);
        });
        it("should handle API errors gracefully", async () => {
            generateText.mockRejectedValue(new Error("API Error"));
            await expect((0, ai_client_1.summarizeDiff)(mockChunks, mockModelConfig)).rejects.toThrow("API Error");
        });
        it("should handle different model providers", async () => {
            ...
            const result = await (0, ai_client_1.summarizeDiff)(mockChunks, anthropicConfig);
...
`;
        ...
        it("should handle non-XML text response", async () => {
            const plainTextResponse = "This is just plain text without any XML structure";
            generateText.mockResolvedValue({
        ...
        it("should handle parsing errors with fallback values", async () => {
            ...
            expect(consoleSpy).toHaveBeenCalledWith("Failed to parse summary response:", expect.any(Error));
...
//# sourceMappingURL=ai-client.test.js.map