...
describe("ReviewService", () => {
    ...
    it("should skip a chunk if it has existing comments", async () => {
        ...
        mockGithubClient.getExistingComments.mockResolvedValue([
            {
                path: "file1.txt",
                line: 2,
                body: `${constants_1.CODEPRESS_REVIEW_TAG} - An existing comment`,
        ...
        expect(mockGithubClient.createReview).toHaveBeenCalledWith(1, "mock-commit-id", [], // No findings
    ...
    it("should process a chunk without existing comments", async () => {
        ...
        expect(mockGithubClient.createReview).toHaveBeenCalledWith(1, "mock-commit-id", mockFindings, expect.objectContaining({
            prType: "feature",
            summaryPoints: ["Test summary point"],
            keyRisks: [],
            hunks: [],
    ...
    it("should pass existing reviews and comments to summarizeDiff for context", async () => {
        ...
        expect(ai_client_1.callWithRetry).toHaveBeenCalledWith(expect.any(Function), 0);
        expect(ai_client_1.callWithRetry).toHaveBeenCalledWith(expect.any(Function), 1);
        ...
        ai_client_1.callWithRetry.mockImplementation((fn, hunkIdx) => {
            if (hunkIdx === 0) {
                return Promise.resolve({
                    ...
                    decision: {
                        recommendation: "REQUEST_CHANGES",
                        reasoning: "Critical security vulnerabilities must be addressed before merge",
                    },
                });
            }
            else {
    ...
    it("should APPROVE PR with findings when decision is APPROVE", async () => {
        ...
        ai_client_1.callWithRetry.mockImplementation((fn, hunkIdx) => {
            if (hunkIdx === 0) {
                return Promise.resolve({
                    ...
                    decision: {
                        recommendation: "APPROVE",
                        reasoning: "Minor suggestions don't block merge. Code quality is good overall and functionality is sound.",
                    },
                });
            }
            else {
        ...
        await reviewService.execute();
        // Assert
        expect(mockGithubClient.createReview).toHaveBeenCalledWith(1, "mock-commit-id", mockFindings, // No hard caps, all optional findings allowed
        expect.objectContaining({
            prType: "feature",
            summaryPoints: ["Added utility functions for data processing"],
            keyRisks: [
...
describe("PR Description Generation", () => {
    ...
    it("should update PR description when feature is enabled and description is generated", async () => {
        ...
        ai_client_1.callWithRetry.mockImplementation((fn, hunkIdx) => {
            if (hunkIdx === 0) {
                ...
                return Promise.resolve({
                    ...
                    prDescription: "## Add Authentication Service\n\nThis PR adds user authentication.",
                });
            }
        ...
        await reviewService.execute();
        // Assert
        expect(mockGithubClient.updatePRDescription).toHaveBeenCalledWith(1, "## Add Authentication Service\n\nThis PR adds user authentication.");
    });
